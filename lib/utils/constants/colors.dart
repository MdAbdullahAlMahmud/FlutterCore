import 'package:flutter/material.dart';
class TColors{
  TColors._();
  
  static const Color primary = Color(0xFF4868FF);
  static const Color seconday = Color(0xFFFFE24B);
  static const Color accent = Color(0xFFb0c7ff);


  static const Color textPrimary = Color(0xFF333333);
  static const Color textSecondary = Color(0xFF6C757D);
  static const Color white = Colors.white;


  // background Color
  static const Color light = Color(0x0ff6f6f6);
  static const Color dark = Color(0xFF272727);
  static const Color primaryBackground = Color(0xFF6C757D);

  //Background containers color
  static const Color lightContainer = Color(0x0ff6f6f6);
  static const Color darkContainer = Color(0xFFD9D9D9);

  //Button Colors
  static const Color buttonPrimary = Color(0xff4b68ff);
  static const Color buttonSecondary = Color(0xff6c7570);
  static const Color buttonDisabled = Color(0x0FFC4C4C4);


  //Border Colors
  static const Color borderPrimary = Color(0xFFD9D9D9);
  static const Color borderSecondary = Color(0xFFE6E6E6);


  static const Color grey = Color(0x0ff6f6f6);






}